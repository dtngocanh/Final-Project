...
import { fetchAllOrders, updateOrderStatus, resetStatus } from "../store/slices/orderSlice"; 
import {
  Eye,
  Trash2,
  ShoppingBag,
...
const Orders = () => {
  ...
  const handleExportCSV = () => {
    if (filteredOrders.length === 0) return toast.warning("No data to export!");
    const headers = ["Order ID", "Customer", "Total", "Method", "Status", "Date"];
      ...
      <div className="relative z-10 p-4 md:p-10 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-6">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-2 font-medium">Manage and track organic produce orders.</p>
          </div>

          <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
            <button onClick={handleExportCSV} className="flex items-center justify-center gap-2 px-6 py-4 bg-white text-gray-700 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all text-xs font-black uppercase tracking-widest active:scale-95">
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-gray-50/50 text-[10px] font-black uppercase tracking-[0.25em] text-gray-400">
                  <tr>
                    ...
                    <th className="px-6 py-6"><CreditCard size={14} className="inline mr-2" /> Payment</th>
                  ...
                  {currentOrders.length > 0 ? (
                    currentOrders.map((order) => (
                      <motion.tr layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={order._id} className="hover:bg-[#fcfdfd] group transition-all">
                        ...
                        {/* HIỂN THỊ THANH TOÁN */}
                        <td className="px-6 py-6">
                            <p className="text-[10px] font-black text-gray-600 uppercase leading-none">{order.paymentInfo?.method || "COD"}</p>
                            <p className={`text-[8px] font-bold mt-1 ${order.paymentInfo?.status === "Paid" ? "text-green-500" : "text-red-400"}`}>
                        ...
                        <td className="px-6 py-6 text-center">
                          <div className="flex justify-center gap-2">
                            <button onClick={() => setSelectedOrder(order)} className="p-3 bg-gray-50 text-gray-400 rounded-2xl hover:bg-gray-800 hover:text-white transition-all transform hover:scale-110 shadow-sm">
                              <Eye size={18} />
                            </button>
                            <button onClick={() => { if(window.confirm("Delete?")) toast.error("Deleted"); }} className="p-3 bg-red-50 text-red-400 rounded-2xl hover:bg-red-500 hover:text-white transition-all transform hover:scale-110 shadow-sm">
                              <Trash2 size={18} />
                            </button>
                          </div>
                        </td>
                    ...
                    <tr>
                      <td colSpan="6" className="text-center py-32 opacity-40 italic">
                        <ShoppingBag size={80} className="mx-auto mb-4 text-gray-200 stroke-1" />
                        Empty stock...
                      </td>
                    </tr>
                  )}
            ...
            {totalPages > 1 && (
              <div className="px-8 py-6 bg-gray-50/30 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setCurrentPage(p => Math.max(p - 1, 1))} disabled={currentPage === 1} className="p-3 rounded-xl bg-white border border-gray-200 text-gray-400 disabled:opacity-30 hover:text-[#77cd3af2] transition-all">
                    <ChevronLeft size={18} />
                  </button>
                  <div className="flex gap-1.5">
                    {renderPageNumbers().map((p, i) => (
                      p === "..." ? <span key={i} className="w-10 h-10 flex items-center justify-center text-gray-400">...</span> :
                      <button key={i} onClick={() => setCurrentPage(p)} className={`w-10 h-10 rounded-xl text-xs font-black transition-all ${currentPage === p ? "bg-[#77cd3af2] text-white shadow-lg" : "bg-white text-gray-400 border border-gray-100"}`}>
                        {p}
                      </button>
                    ))}
                  </div>
                  <button onClick={() => setCurrentPage(p => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages} className="p-3 rounded-xl bg-white border border-gray-200 text-gray-400 disabled:opacity-30 hover:text-[#77cd3af2] transition-all">
