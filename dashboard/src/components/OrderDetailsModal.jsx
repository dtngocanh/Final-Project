...
import { 
  ...
  Salad, Citrus, Cherry, Leaf, Carrot 
} from "lucide-react";
...
const OrderDetailsModal = ({ order, onClose }) => {
  ...
  // Định nghĩa rau củ decor bằng Lucide Icons
  const decoVeggie = [
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white/95 w-full max-w-2xl rounded-[40px] shadow-2xl overflow-hidden relative font-['Fredoka'] border border-white/20">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          {decoVeggie.map((veg, i) => (
            <motion.div
              ...
              animate={{
                y: [0, -25, 0],
                rotate: [veg.rotate, veg.rotate + 10, veg.rotate - 10, veg.rotate],
              }}
              transition={{
        ...
        <div className="relative z-10">
          {/* Header */}
          <div className="bg-[#77cd3af2]/90 backdrop-blur-md p-8 text-white flex justify-between items-start font-['Fredoka']">
            <div>
            ...
            <button 
              onClick={onClose} 
              className="p-2 bg-white/20 rounded-full hover:bg-white/40 transition-all hover:rotate-90"
            >
              <X size={20} />
            </button>
          </div>
          ...
          <div className="p-8 max-h-[60vh] overflow-y-auto custom-scrollbar relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 font-['Fredoka']">
              <div className="space-y-3 bg-white/60 p-5 rounded-[30px] border border-gray-100 shadow-sm backdrop-blur-sm">
                ...
                <div>
                  ...
                  <p className="flex items-center gap-2 text-gray-400 text-xs italic mt-1 font-medium underline decoration-dotted">
                    <Mail size={12} /> {order.user?.email || "customer@veganic.com"}
                  </p>
            ...
            <div className="space-y-4 font-['Fredoka']">
              ...
              <div className="space-y-3">
                {order.orderItems?.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center bg-white/80 p-3 rounded-[24px] shadow-sm border border-gray-50 group hover:border-[#77cd3af2] transition-all backdrop-blur-sm">
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        <img 
                          src={item.image || "https://cdn-icons-png.flaticon.com/512/2329/2329865.png"} 
                          alt={item.name}
                      ...
                      <div>
                        <span className="font-bold text-gray-700 block text-base font-['Fredoka']">{item.name}</span>
                        <span className="text-[10px] text-gray-400 font-bold tracking-tighter uppercase font-['Fredoka']">${item.price} / unit</span>
                      </div>
                    </div>
                    <span className="font-black text-[#77cd3af2] text-xl font-['Fredoka'] tracking-tight">
          ...
          <div className="p-8 bg-white/90 backdrop-blur-md border-t border-gray-50 flex flex-col md:flex-row justify-between items-center gap-4 font-['Fredoka']">
            ...
            <button 
              onClick={onClose} 
              className="w-full md:w-auto px-12 py-4 bg-gray-900 text-white rounded-[22px] font-bold text-sm hover:bg-[#77cd3af2] transition-all shadow-xl active:scale-95 shadow-gray-200"
            >
              Confirm & Close
            </button>
          </div>
