      ...
      <div className="max-w-[1400px] mx-auto px-6">
        ...
        <header className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold tracking-tight dark:text-white">
              Special <span className="font-serif italic text-[#77cd3a]">Deals</span> for you
            </h2>
          </div>
          <Link to="/" className="text-[10px] uppercase tracking-widest font-bold text-neutral-400 hover:text-[#77cd3a]">
            View All
          </Link>
        </header>
        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4">
          {isLoading ? (
             ...
             [...Array(4)].map((_, i) => (
               <div key={i} className="min-w-[200px] h-[250px] bg-gray-100 animate-pulse rounded-2xl" />
             ))
          ) : (
              ...
              >
                <Link to={`/product/${product._id}`}>
                  ...
                  <div className="space-y-1">
                    <h4 className="text-xs font-bold text-neutral-800 line-clamp-1">{product.name}</h4>
                    <div className="flex justify-between items-center">
                      <div className="flex flex-col">
                        <span className="text-sm font-black text-[#77cd3a]">
                          ${(product.price * 0.9).toFixed(2)} {/* Giá đã giảm 10% */}
                        </span>
                      ...
                      <div className="flex items-center gap-1 bg-gray-50 px-1.5 py-0.5 rounded-md">
                        ...
                        <span className="text-[9px] font-bold text-neutral-600">4.9</span>
                      </div>
                    </div>
                  </div>
                </Link>
