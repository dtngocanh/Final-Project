...
const LoginModal = () => {
  ...
  const vegies = [
    ...
    { Icon: Cherry, size: 70, bottom: '10%', right: '0%', delay: 1, rotate: -10 },
  ...
  const handleSubmit = (e) => {
    ...
    if (mode === "forgot") {
      dispatch(forgotPassword({ email: formData.email })).then(() => setMode("signin"));
      return;
    }
        ...
        >
          ...
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            {vegies.map((item, index) => (
              ...
              >
                <item.Icon size={item.size} strokeWidth={0.6} style={{ color: activeColor }} />
              </motion.div>
            ))}
          </div>

          <button onClick={handleClose} className="absolute top-6 right-6 p-2 text-gray-400 hover:text-[#77cd3af2] transition-transform hover:scale-110 z-20">
            <X size={22} />
          </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              {mode === "signup" && (
                <div className="relative group border-b border-gray-200 dark:border-white/10 focus-within:border-[#77cd3af2] transition-colors">
                  <User className="absolute left-0 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#77cd3af2] transition-colors" size={18} />
                  <input
