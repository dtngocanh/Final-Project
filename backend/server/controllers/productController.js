...
export const productList = async (req, res, next) => {
  try {
    ...
    const [totalProducts, products] = await Promise.all([
      ...
      Product.find(query)
        .select("name description price images category stock ratings discountPrice salesCount")
...
export const productById = async (req, res, next) => {
  try {
    ...
    const product = await Product.findById(id)
      .select("name price images category ratings stock description numOfReviews discountPrice salesCount")
      .lean();
