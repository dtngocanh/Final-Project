...
import { openModalWithCategory } from "../../store/slices/campaignsSlice"; // Import Action mới
import PurchaseOrderModal from "./PurchaseOrderModal";
import ExpiryAlerts from "./ExpiryAlerts";
import { Package, ArrowLeft, DollarSign, AlertCircle, TrendingDown, Tag } from "lucide-react";
...
const InventoryOverview = () => {
  ...
  // Tối ưu useMemo để lấy cả ID danh mục phục vụ tạo Promo
  const stats = useMemo(() => {
    ...
    products.forEach((product) => {
      ...
      // Lấy tên danh mục và object chứa ID gốc
      const isObjCategory = typeof product.category === "object" && product.category?._id;
      const categoryName = isObjCategory ? product.category.name : product.categoryName || "Uncategorized";
      const categoryId = isObjCategory ? product.category._id : "uncategorized";

      totalQty += stock;
      if (stock === 0) stockOutCount++;

      // Dùng categoryId làm KEY chính xác để không bị trùng lặp, lưu kèm rawCategory phục vụ truyền Redux
      if (!categoryData[categoryId]) {
        categoryData[categoryId] = {
          ...
          value: 0,
          totalSold: 0,
          rawCategory: isObjCategory ? product.category : { _id: "uncategorized", name: "Uncategorized" }
        };
      }

      categoryData[categoryId].qty += stock;
      categoryData[categoryId].value += stock * price;
      categoryData[categoryId].totalSold += sold;
    });

    const lowStockItems = [...products].filter((p) => p.stock < 10).slice(0, 6);

    return { totalQty, totalValue, stockOutCount, lowStockItems, categoryData };
  }, [products, restockLogs]);
        ...
        {topStats.map((stat, idx) => {
          ...
          return (
            <motion.div
              ...
              className={`relative overflow-hidden rounded-[32px] bg-gradient-to-br ${stat.ring} border border-white/60 backdrop-blur-sm p-6 shadow-sm hover:shadow-xl`}
            >
              ...
              <div className="relative z-10">
                <div className="flex items-start justify-between mb-6">
                  ...
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm ${stat.color}`}>
                    <Icon size={26} />
                  </div>
                </div>
                ...
                {stats.lowStockItems.length > 0 ? (
                  stats.lowStockItems.map((prod, index) => (
                    <tr key={index} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className={`py-3.5 text-right font-bold text-sm ${prod.stock === 0 ? "text-rose-500 font-black" : "text-amber-500"}`}>
        ...
        <div className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm col-span-1 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="font-bold text-gray-800 text-lg">Recent Restock Log</h3>
            ...
            <p className="text-xs text-gray-400 mb-6">Latest stock replenishment activities</p>
            <div className="space-y-4 max-h-[240px] overflow-y-auto pr-1 custom-scrollbar">
              {restockLogs && restockLogs.length > 0 ? (
                restockLogs.map((log, idx) => (
                  <div key={log._id || idx} className="flex items-center justify-between p-3 rounded-2xl bg-gray-50/60 hover:bg-emerald-50/30 border border-gray-100/50 transition-all group">
                    <div className="flex flex-col gap-0.5 min-w-0 flex-1">
                      <span className="text-[10px] text-gray-400 font-bold">#{log._id?.slice(-6).toUpperCase()}</span>
                      <p className="text-xs font-bold text-gray-800 truncate group-hover:text-gray-900">{log.product?.name || "Deleted Product"}</p>
                      <p className="text-[10px] text-gray-400 font-medium">From: {log.supplier}</p>
                    </div>
                    <div className="text-right shrink-0 ml-3">
                      <span className="text-xs font-black text-emerald-600 block">+{log.quantityAdded} items</span>
                ...
                <div className="text-center py-10 text-gray-400 italic text-xs">No recent inbound logs found.</div>
              )}
            </div>
          </div>
          <button
          ...
          <PurchaseOrderModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); dispatch(restockProductLogs()); }} products={products} />
        </div>

        {/* BLOCK 3: CATEGORY ALLOCATION (CÓ NÚT PROMO CHO TỪNG CATEGORY) */}
        <div className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm col-span-1 flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-gray-800 text-lg mb-1">Category Allocation</h3>
            <p className="text-xs text-gray-400">Detailed breakdown of quantity, inventory value, and sales velocity</p>
          </div>

          <div className="space-y-5 my-4 max-h-[320px] overflow-y-auto pr-1 custom-scrollbar">
            {Object.keys(stats.categoryData).length > 0 ? (
              Object.entries(stats.categoryData).map(([catId, catObj], idx) => {
                const percentage = stats.totalQty ? ((catObj.qty / stats.totalQty) * 100).toFixed(1) : 0;
                ...
                } else if (catObj.qty > 0 && catObj.totalSold <= 5) {
                  velocityBadge = (
                ...
                return (
                  <div key={catId} className="text-xs border-b border-gray-50/50 pb-3 last:border-0 last:pb-0">
                    <div className="flex justify-between items-start mb-1.5">
                      <div>
                        {/* Thay vì chỉ hiển thị text, bọc khối tên và thêm nút Promo xinh xắn kế bên */}
                        <div className="flex items-center gap-2">
                          <span className="text-gray-800 font-bold capitalize text-[13px] block">
                            {catObj.name}
                          </span>
                          
                          {/* CHÈN NÚT PROMO TRUYỀN ID CATEGORY TẠI ĐÂY */}
                          {catId !== "uncategorized" && (
                            <button
                              type="button"
                              onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                // Đẩy Object chứa _id và name danh mục lên Redux Store
                                dispatch(openModalWithCategory(catObj.rawCategory));
                                // Chuyển hướng ngay tới trang quản lý chiến dịch
                                navigate("/campaigns");
                              }}
                              className="bg-[#77cd3af2] hover:bg-[#6ab933] text-white text-[9px] font-black px-2 py-0.5 rounded-md flex items-center gap-1 cursor-pointer tracking-wider transition-all shadow-sm active:scale-95"
                            >
                              <Tag size={10} /> PROMO
                            </button>
                          )}
                        ...
                        <div className="flex gap-2 mt-1 items-center">
                          {velocityBadge}
                          <span className="text-[10px] text-gray-400">Sold: {catObj.totalSold}</span>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className="text-gray-900 font-black block text-[13px]">
                          {catObj.qty.toLocaleString()} pcs ({percentage}%)
                        </span>
                      </div>
                    </div>
                    {/* Progress Bar */}
                    <div className="w-full h-2 bg-gray-50 rounded-full overflow-hidden border border-gray-100 mt-2">
                      <div
                        className={`h-full rounded-full ${catObj.totalSold > 50 ? "bg-emerald-500" : "bg-[#77cd3af2]"}`}
              ...
              <div className="text-center py-10 text-gray-400 italic">No category data found</div>
            )}
          </div>
        </div>
      </div>
