...
const ProductSlider = ({
  ...
  // CHỖ THAY ĐỔI 1: TỰ ĐỘNG RENDERING TAG GIẢM GIÁ THỜI GIAN THỰC THEO ATLAS TRIGGER
  const renderProductTag = (product) => {
    ...
    const isSaleActive = product.discountPrice && product.discountPrice > 0;

    if (!tagText) {
      if (isSaleActive) {
        // Tính nhẩm phần trăm giảm dựa trên giá gốc và giá discountPrice thực tế trong database
        const calculatedDiscount = Math.round(
          ((product.price - product.discountPrice) / product.price) * 100
        );
        tagText = `-${calculatedDiscount}%`;
        tagClass = "bg-rose-500 text-white font-medium animate-pulse"; // Thêm nhấp nháy cho chiến đét
  ...
  useEffect(() => {
    if (products && products.length > 0) {
      console.log("=== SOI DATA SẢN PHẨM ĐẦU TIÊN ===");
      console.log("Name:", products[0].name);
      console.log("Price (Giá gốc):", products[0].price);
      console.log("DiscountPrice (Giá sale):", products[0].discountPrice);
    }
    }, [products]);
          ...
          {chunkedProducts.map((pair, pairIdx) => (
            ...
            >
              {pair.map((product, idx) => {
                // Kiểm tra nhanh xem sản phẩm hiện tại có đang được Flash Sale hay không
                const isSaleActive = product.discountPrice && product.discountPrice > 0;
                ...
                return (
                  ...
                  >
                    <div className="relative flex flex-col bg-white dark:bg-neutral-900 rounded-2xl sm:rounded-[2rem] overflow-hidden border border-neutral-100 dark:border-neutral-800/60 shadow-sm lg:hover:border-[#77cd3a]/40 lg:hover:shadow-[0_20px_40px_rgba(119,205,58,0.06)] transition-all duration-300 transform-gpu">
                      ...
                      {/* CHỖ THAY ĐỔI 2: HIỂN THỊ ĐỒNG THỜI GIÁ SALE (ĐẬM) VÀ GIÁ GỐC (GẠCH NGANG) */}
                      <div className="px-3 py-4 sm:px-6 sm:py-6 text-center relative z-20 bg-transparent">
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-1.5">
                          {isSaleActive ? (
                            <>
                              {/* Giá Flash Sale hiển thị chính thức */}
                              <span className="text-xs sm:text-sm font-bold text-rose-500 dark:text-rose-400">
                                ${product.discountPrice?.toFixed(2)}
                              </span>
                              {/* Giá gốc cũ bị gạch ngang */}
                              <span className="text-[10px] sm:text-xs text-neutral-400 line-through font-normal">
                                ${product.price?.toFixed(2)}
                              </span>
                            </>
                          ) : (
                            /* Không sale -> Hiện giá gốc bình thường */
