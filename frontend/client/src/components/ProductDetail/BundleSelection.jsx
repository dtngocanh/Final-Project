...
const BundleSelection = ({ mainProduct }) => {
  ...
  // --- ĐOẠN TÍNH TOÁN GIÁ ĐỘNG TẠI FRONTEND ĐỂ HIỂN THỊ ---
  const originalTotalPrice =
    (mainProduct?.price || 0) +
    activeBundleItems.reduce((sum, item) => sum + (item.price || 0), 0);

  // Điều kiện giảm giá: Chọn đủ combo 3 món (Món chính + 2 món phụ được active)
  ...
  // --- LUỒNG GỬI COMBO LÊN BACKEND BẢO MẬT ---
  const handleAddBundleToCart = async () => {
    if (!isEligibleForDiscount) return;

    // Gói dữ liệu cần thiết truyền vào Thunk
    const payload = {
      mainProductId: mainProduct._id,
      comboProductIds: selectedItems, // Mảng các ID sản phẩm đi kèm
    };

    // Dispatch thunk action trực tiếp lên Redux
    const resultAction = await dispatch(addComboToCart(payload));

    // Kiểm tra kết quả trả về để thông báo cho người dùng
    if (addComboToCart.fulfilled.match(resultAction)) {
      toast.success("Added full combo with 10% discount! 🛒");
    } else {
      toast.error(resultAction.payload || "Failed to add combo");
    }
  };
          ...
          <div className="flex items-center justify-center gap-3 md:gap-6 flex-[1.5] bg-neutral-50/50 dark:bg-white/[0.01] rounded-[2.5rem] py-8 px-4 border border-neutral-100 dark:border-white/5">
            ...
            {limitedFreqProducts.map((item) => (
              <React.Fragment key={item.productId}>
                <Plus
                  size={18}
                  className="text-neutral-300 stroke-[1.5px] flex-shrink-0"
                />
                <BundleItem
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 pt-4">
                <div className="flex justify-between text-[13px] dark:text-white group">
                  <span className="truncate opacity-60 w-3/4 group-hover:opacity-100 transition-opacity italic">
                    1x {mainProduct?.name}
                  </span>
              ...
              {/* THÀNH PHẦN HIỂN THỊ ĐỘNG: GIẢM GIÁ VÀ TIẾT KIỆM */}
              <div className="pt-6 border-t border-dashed border-neutral-200 dark:border-white/10 space-y-2">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xs font-black uppercase dark:text-white tracking-wider">
                    Total Amount
                  </span>
                  <div className="text-right">
                    ...
                    {isEligibleForDiscount && (
                      <span className="text-[11px] text-[#77cd3a] block font-medium">
...
// Component con BundleItem giữ nguyên cấu trúc đẹp mắt của bạn...
const BundleItem = ({
  image,
  isMain = false,
  isSelected = true,
  ...
  <div className="flex flex-col items-center gap-4">
    ...
    >
      ...
      {!isMain && (
        <div
          className={`absolute -top-1 -right-1 w-7 h-7 rounded-full flex items-center justify-center border-[3px] border-white dark:border-[#060606] transition-all duration-500 ${
            isSelected
              ? "bg-[#77cd3a] text-white scale-100"
              : "bg-neutral-300 scale-0"
          }`}
        >
    ...
    <span
      className={`text-[9px] uppercase font-bold tracking-tight text-center max-w-[80px] leading-tight transition-colors ${isSelected ? "text-neutral-500" : "text-neutral-300"}`}
    >
